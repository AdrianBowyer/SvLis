...
class sv_interval
{
 private:
  ...
  // An interval is represented as (a0 + od*[-1,1] + ev*[0,1])
...
// I/O and arithmetic functions that need access to the private data	

	friend void write(ostream& s, const sv_interval& i, sv_integer level);
	friend void read(istream& s, sv_interval& i);
        friend sv_interval operator-(const sv_interval& a);
	friend sv_interval operator+(const sv_interval& a, sv_real b);
	friend sv_interval operator*(const sv_interval& a, sv_real b);
	friend sv_interval operator+(const sv_interval& a, const sv_interval& b);
	friend sv_interval operator*(const sv_interval& a, const sv_interval& b);
	friend sv_interval pow(const sv_interval& a, sv_integer i);
};
...
inline sv_interval operator*(const sv_interval& a, const sv_interval& b)
{
  ...
  sv_interval result;
  result.a0 = a.a0*b.a0;
  result.od = fabs(a.a0*b.od + b.a0*a.od + a.od*b.ev + a.ev*b.od);
  result.ev = a.a0*b.ev + a.od*b.od + a.ev*b.a0 + a.ev*b.ev;
  return(result);
#else
	sv_real c = a.lo()*b.lo();
	sv_real d = c;
...
inline sv_interval abs(const sv_interval& a)
{
	...
	if (a.lo() < 0.0)
	{
		if (a.hi() <= 0.0)
		{
		  result = sv_interval(-a.hi(), -a.lo());
		} else
		{
			result = sv_interval(0.0, result.hi());
			if ( (p = -a.lo()) > a.hi() ) result = sv_interval(result.lo(), p);
		}
	}
	return(result);
}
...
inline sv_interval max(const sv_interval& a, const sv_interval& b)
{
	...
	if (a.lo() > b.lo()) result = sv_interval(a.lo(), result.hi());
	if (a.hi() > b.hi()) result = sv_interval(result.lo(), a.hi());
	return(result);
}
