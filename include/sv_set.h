...
// Functions to return the data
	...
	sv_point grad(const sv_point&, sv_real&) const;
...
// 1D Scale along a line
	...
	sv_set scale(const sv_line&, sv_real) const;
...
// Needed for the faceter

	friend sv_integer contents_4(const sv_model& mm, const sv_set& s, int force);
...
// Grad of a set (returns value as well as a side-effect)

inline sv_point sv_set::grad(const sv_point& p, sv_real& v) const
{
  sv_set w;
  v = this->value(p, &w);
  return(w.primitive().grad(p));
}
...
// Name is the name of the first set

	char* name() const { return(set().name()); }
