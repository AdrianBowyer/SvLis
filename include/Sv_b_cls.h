...
public:
  sv_lock() 
  {
	  sv_mutex = CreateMutex( NULL, FALSE, NULL );
	  if(!sv_mutex)
		  svlis_error("sv_lock()", "Mutex creation failed.", SV_FATAL);
  }
  ~sv_lock()
  {
	  //if(!ReleaseMutex(sv_mutex))
		  //svlis_error("~sv_lock()", "Mutex release failed.", SV_WARNING);
	  if(!CloseHandle(sv_mutex))
		  svlis_error("~sv_lock()", "Close handle failed.", SV_WARNING);
  }
  void shut() 
  {
	  if(WaitForSingleObject( sv_mutex, INFINITE ) == WAIT_FAILED)
		svlis_error("lock::shut()", "Wait failed.", SV_WARNING);
  }
  void open()
  {
	  if(!ReleaseMutex(sv_mutex))
		  svlis_error("lock::open()", "Mutex release failed.", SV_WARNING);
  }
};
