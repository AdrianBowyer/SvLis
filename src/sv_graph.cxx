...
{ 
   ...
   ray_pic.resolution(round(ww), round(wh)); 
...
{ 
  ...
  { 
	...
	{ 
		...
		if(!ib.empty()) 
		{ 
			if(ib.lo() < 0) ib = sv_interval(0, ib.hi()); 
			if(!ib.empty()) 
			{ 
...
// Remember current window
   ...
   glReadPixels(0, 0, wx, hy, GL_RGB, GL_UNSIGNED_BYTE, &(result->image_bitmap->r));
...
// Invert image
   ...
   sv_pixel temp;
   sv_integer a0, a1;
   for(sv_integer j = 0; j < hy/2; j++)
    for(sv_integer i = 0; i < wx; i++)
    {
        a0 = i + j*result->x_res;
        a1 = i + (hy - j)*result->x_res;
	temp = result->image_bitmap[a0];
        result->image_bitmap[a0] = result->image_bitmap[a1];
        result->image_bitmap[a1] = temp;
    }
