...
#else
	if(get_token(s, id, i.l, 0) != SVT_REAL)
		svlis_error("read(.. sv_interval)","dud low token", SV_WARNING);
	if(get_token(s, id, i.h, 0) != SVT_REAL)
		svlis_error("read(.. sv_interval)","dud high token", SV_WARNING);
#endif
