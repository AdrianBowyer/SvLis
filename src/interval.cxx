...
#else
        put_token(s, SVT_REAL, 0, i.l);
        s << ", ";
        put_token(s, SVT_REAL, 0, i.h);
#endif
        s << ", ";
...
istream& operator>>(istream& s, sv_interval& i)
{
	...
	else
	{
		if(get_read_version() != (SV_VER-1))
			svlis_error("operator>> sv_interval",
...
prim_op same(const sv_interval& a, const sv_interval& b)
{
...
sv_interval pow(const sv_interval& a, sv_integer i)
{
  ...
  if(i == 2) // No more accurate, but saves time
  {
    result.a0 = a.a0*a.a0;
    result.od = fabs(2*(a.a0*a.od + a.od*a.ev));
    result.ev = 2*a.a0*a.ev + a.od*a.od + a.ev*a.ev;
    result.empt = 0; 
  } else
  {
    result = sv_interval(1,1);
  ...
  return(result);
	...
	if (i < 0)
	{
		svlis_error("sv_interval::operator^","negative exponent",SV_WARNING);
		return(a);
	}

	if (i%2)
	{
		return(sv_interval(pow(a.lo(),i),pow(a.hi(),i)));
	} else
	{
		if ((a.lo() < 0.0) && (a.hi() < 0.0)) 
			return(sv_interval(pow(a.hi(),i),pow(a.lo(),i)));
		if ((a.lo() < 0.0) && (a.hi() >= 0.0))
			return(sv_interval(0,pow(max(-a.lo(), a.hi()), i)));
		return(sv_interval(pow(a.lo(),i),pow(a.hi(),i)));
	}
#endif
}
...
sv_interval sin(const sv_interval& uu)
{
/*
...
// First test if interval is greater than a revolution.
   ...
   if ((u.hi() - u.lo()) >= (2*M_PI)) return (sv_interval(-1.0, 1.0));
 ...
 if( ((u.lo() <= 0.5*M_PI) && (u.hi() >= 1.5*M_PI)) ||
 ...
 else
  {
   ...
   if( ((u.lo() <= 0.5*M_PI) && (u.hi() >= 0.5*M_PI)) ||
     ...
     {
      v = sv_interval((sv_real)sin(u.lo()), v.hi());
      if( (x = (sv_real)sin(u.hi())) < v.lo() ) v = sv_interval(x, v.hi());
      v = sv_interval(v.lo(),1.0);
     }
   else
     {
      ...
      if( ((u.lo() <= 1.5*M_PI) && (u.hi() >= 1.5*M_PI)) ||
        ...
        {
         v = sv_interval(v.lo(), (sv_real)sin(u.lo()));
         if( (x = (sv_real)sin(u.hi())) > v.hi() ) v = sv_interval(v.lo(), x);
         v = sv_interval(-1.0, v.hi());
        }
      else
        {
         /* else compute sin values */
         v = sv_interval((sv_real)sin(u.lo()), (sv_real)sin(u.hi()));
	 if (v.lo() > v.hi())
	 {
		x = v.lo();
		y = v.hi();
...
{
	sv_interval result;	// Null constructor makes an empty interval

	if (a.empty() || b.empty()) return(result);
	if (a.lo() > b.hi()) return(result);
	if (b.lo() > a.hi()) return(result);
	if (b.lo() <= a.lo())
		result = sv_interval(a.lo(), result.hi());
	else 
		result = sv_interval(b.lo(), result.hi());
	if (b.hi() >= a.hi())
		result = sv_interval(result.lo(), a.hi());
	else
		result = sv_interval(result.lo(), b.hi());
	return(result);
}
...
sv_interval operator|(const sv_interval& a, const sv_interval& b)
{
	...
	} else
	{
		if(b.empty()) return(a);
		result = a;
		if (b.lo() < a.lo()) result = sv_interval(b.lo(), result.hi());
		if (b.hi() > a.hi()) result = sv_interval(result.lo(), b.hi());
	}
	return(result);
}
...
sv_interval l_int_plane(const sv_line& l, sv_plane& f, const sv_interval& i)
{
	...
	if (f.value(l_start) < f.value(l_end))
		result = sv_interval(result.lo(), min(t,i.hi()));
	else
		result = sv_interval(max(t,i.lo()), result.hi());
